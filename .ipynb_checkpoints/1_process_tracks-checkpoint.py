"""
Thank you to Miaorui Yu from Nanjing University for sharing the track processing code. - Sarah
...
]  # SH: edited to process the North Atlatnic only

# settings
YEAR_START = 1980
TCG_DATA_DIR = './private/MATCH-NH-Xiangbo'
...
FINAL_OUTPUT_FILE = './processed_tracks/TCG_refined.xlsx'
...
tcg_sheets = pd.read_excel(TCG_PATH, sheet_name=None)
tcg_df = pd.concat(tcg_sheets.values(), ignore_index=True)
...
ib_df = ib_df.sort_values('time').reset_index(drop=True)

# Numpy arrays for fast filtering
...
REFINED_TCG_FILE = "./processed_tracks/TCG_refined.xlsx"     # output of Step 2
CLEANED_DEVELOPER_FILE = "./processed_tracks/developer_refined.xlsx"
refined_tcg_df = pd.read_excel(REFINED_TCG_FILE)
...
removed_dev_ids = original_dev_ids - remaining_dev_ids

# Save 
cleaned_developer_df.to_excel(CLEANED_DEVELOPER_FILE, index=False)
print(f"[INFO] Refined developer dataset saved to {CLEANED_DEVELOPER_FILE}")


#%% Step 4: Extract Non-Developers from Raw Vortex Tracks - SH added to process non-developers

print("\n[INFO] ===== Step 4: Processing Non-Developers =====")

NON_DEVELOPER_FILE = "./processed_tracks/non_developers.xlsx"
...
def process_raw_vortex_tracks(file_path, year):
    """Process raw vortex tracks and extract first point (seed) of each track"""
    with open(file_path, 'r') as f:
        # Skip headers
        f.readline()
        f.readline()
        ...
        for _ in range(track_count):
            ...
            # Only process FIRST point (seed)
            t_val = int(track_data[0, 0])
            lon = float(track_data[1, 0])
            lat = float(track_data[2, 0])
            rv = float(track_data[3, 0])
            ...
            # Check which basin contains this seed
            p = Point(lon, lat)
            for b_idx, poly in enumerate(BASIN_POLYGONS):
                if p.within(poly):
                    all_seeds_dict[BASIN_NAMES[b_idx]] = pd.concat([
                        all_seeds_dict[BASIN_NAMES[b_idx]],
                        pd.DataFrame([{
                            "track_id": track_id,
                            "id": int(str(track_id) + str(year_val)),
                            "time": t_val,
                            "lat": lat,
                            "lon": lon,
                            "year": year_val,
...
for year_offset in range(43):  # 1980–2022
    year = YEAR_START + year_offset
    file_path = os.path.join("./private/seeds_Track/NH", f'{year}_NH{file_nam_n}')
    
    if os.path.exists(file_path):
        process_raw_vortex_tracks(file_path, year)
    else:
...
print(f"[INFO] Total unique seeds extracted from raw vortex tracks: {total_seeds}")
...
non_developers_df = all_seeds_df[
    ~all_seeds_df["id"].isin(developer_ids)
].reset_index(drop=True)

non_dev_count = non_developers_df["id"].nunique()
...
print(f"- Non-developers              : {non_dev_count}")
print(f"- Verification (should be 0)  : {total_seeds - len(developer_ids & set(all_seeds_df['id'].unique())) - non_dev_count}")

# Save non-developers
non_developers_df.to_excel(NON_DEVELOPER_FILE, index=False)
print(f"[INFO] Non-developer dataset saved to {NON_DEVELOPER_FILE}")
...
print(f"- Non-developers         : {NON_DEVELOPER_FILE}")  # SH - added to summarize non-developer processing
